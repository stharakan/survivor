...
// External ID Backfill Script for 2026/2027 EPL Season
// This script backfills external IDs for all EPL 2026/2027 games by matching them against Football Data API
...
// Fetch all EPL matches for the 2026 season from Football Data API
async function fetchEPLFixtures(): Promise<any[]> {
  logWithTimestamp(`Fetching EPL ${SEASON_DISPLAY} fixtures from Football Data API...`)
...
async function getGamesNeedingExternalIds(): Promise<any[]> {
  logWithTimestamp(`Finding ${SEASON_DISPLAY} games without external IDs...`)
...
async function findTeamByApiName(apiTeam: { name: string, shortName: string }): Promise<any | null> {
  ...
  // If multiple teams found, prefer the one that's actually used in the target season's games
...
async function findMatchingApiGame(dbGame: any, apiFixtures: any[]): Promise<any | null> {
  ...
  for (const apiGame of apiFixtures) {
    ...
    // Widened to 4 days (was 2) to catch Monday-night fixtures whose DB date
    // is stored as the preceding Saturday 00:00 UTC — a real matchweek can span
    // Fri-Mon, up to ~3.5 days peak-to-peak.
...
async function updateGameWithExternalId(dbGame: any, apiGame: any): Promise<{ success: boolean, error?: string }> {
  try {
    if (DRY_RUN) {
      logWithTimestamp(`[DRY RUN] Would update game ${dbGame.id} with external ID: ${apiGame.id}`)
      return { success: true }
    }
...
async function backfillExternalIds(dbGames: any[], apiFixtures: any[]): Promise<{
  ...
  for (const dbGame of dbGames) {
    ...
    // No inter-game sleep: the Football Data API is called exactly once (bulk
    // fetch at start), so per-game rate limiting is unneeded here.
  }

  return results
}

// Verify that all target-season games now have external IDs
async function verifyExternalIds(): Promise<{ total: number, withExternalId: number, missing: number }> {
  ...
  logWithTimestamp(`Total ${SEASON_DISPLAY} games: ${totalGames}`)
...
async function main() {
  ...
  try {
    logWithTimestamp(`=== External ID Backfill Started ${DRY_RUN ? '(DRY RUN — no writes)' : ''} ===`)
    ...
    if (initialVerification.missing === 0) {
      logWithTimestamp(`✓ All ${SEASON_DISPLAY} games already have external IDs. No backfill needed.`)
      process.exit(0)
    }
    ...
    logWithTimestamp(`  • ${finalVerification.total} total ${SEASON_DISPLAY} games`)
    ...
    if (finalVerification.missing === 0) {
      logWithTimestamp(`\n🎉 SUCCESS: All ${SEASON_DISPLAY} games now have external IDs!`)
      process.exit(0)
    } else {
