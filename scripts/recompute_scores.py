"""Manually trigger scoring. Same env conventions as update_game_scores.py.
...
Usage:
    uv run --project . python scripts/recompute_scores.py           # dev
    uv run --project . python scripts/recompute_scores.py --prod    # prod
"""
import asyncio
import json
import sys
...
from app.db.scoring import run_scoring_calculation  # noqa: E402
...
async def main() -> int:
    ...
    log(f"=== Score Recomputation Started ({env_label}) ===")
    try:
        result = await run_scoring_calculation()
        log("=== Score Recomputation Completed ===")
