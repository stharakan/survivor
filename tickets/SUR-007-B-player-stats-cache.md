...
**Ticket ID**: SUR-007-B
**Title**: Implement Scoreboard Stats Cache for Performance Optimization
...
As a league administrator and player, I want a fast-loading scoreboard page, so that I can quickly view standings even as league participation grows to 50+ players.

## Description

### Context and Background

Currently, the scoreboard page performs expensive database operations with multiple collection joins and aggregations on every page load. This creates performance bottlenecks as leagues grow.

This ticket implements a materialized view pattern using a `player_stats_cache` collection to pre-compute aggregate player statistics and rankings, dramatically reducing scoreboard load times.

**Scope Note**: This cache serves the scoreboard page only. Results and profile pages continue to use live queries, as the scoreboard is the most frequently loaded and most expensive page to render at scale.

### Current State vs. Desired State

**Current:**
- Every scoreboard page load triggers complex multi-collection aggregations
...
**Desired:**
- Materialized view (`player_stats_cache`) updated once during score calculation
- Scoreboard reads from cached collection with a single query (no joins)
- Pre-computed rankings stored in cache
- Cache invalidation only during automated scoring runs (every 15 minutes)
- Page load time: < 200ms for scoreboard
...
- **Document Size**: ~500 bytes per player (flat scalar fields only), trivial relative to MongoDB 16MB limit
...
- Each document contains: user info and aggregate stats (flat scalar fields only)
...
- `survivor_status` correctly reflects whether the player has met the league's total strike limit
...
- Displays: rank, teamName, userName, points, strikes, survivor_status
- Strike breakdown available: "X total (Y losses, Z missing)"

### AC3: Source of Truth Preservation
...
### AC5: Rank Calculation with Tie Handling
...
export interface PlayerStatsCache {
  ...
  survivor_status: 'surviving' | 'eliminated';  // Whether player has met the league's total strike limit
...
// For cache updates (upsert by league + user)
...
**No Breaking Changes** - Existing API continues to work, just reads from different source:

**Modified Endpoint:**
- `GET /api/leagues/[leagueId]/scoreboard` → Queries `player_stats_cache` with rank sort
 ...
 * Reads aggregate stats from league_memberships and user info from users
 */
...
describe('updatePlayerStatsCache', () => {
  ...
  it('copies aggregate stats from league_memberships correctly', async () => {
    // Given: Player with 15 points, 3 strikes (2 loss, 1 missing)
    // When: Cache update runs
    // Then: Cache document reflects same values
  })

  it('sets survivor_status to eliminated when strikes meet limit', async () => {
    // Given: League with 3-strike limit, player has 3 total strikes
    // When: Cache update runs
    // Then: survivor_status = 'eliminated'
  })

  it('sets survivor_status to surviving when under strike limit', async () => {
    // Given: League with 3-strike limit, player has 2 total strikes
    // When: Cache update runs
    // Then: survivor_status = 'surviving'
  })
})
...
describe('Full Cache Update Flow', () => {
  ...
  it('scoreboard reflects cached data immediately', async () => {
    // Given: Cache has been updated
    // When: Scoreboard page loads
    // Then: Shows data from cache, not source
  })
})
```
...
# Should complete in < 2 seconds for 100 players
```
...
- Type names: PascalCase (`PlayerStatsCache`)
...
3. Create TypeScript types in `types/player-stats-cache.ts`
4. Modify `runScoringCalculation()` to call cache update
5. Deploy - cache starts populating on next scoring run

**Phase 2: Update Scoreboard Read Path (Feature Flag)**
6. Add environment variable `USE_PLAYER_STATS_CACHE=true|false` (default: false)
7. Update scoreboard API route to check flag: if true, read cache; else, read source
...
14. Add cache monitoring/alerting
...
- [ ] Scoreboard page reads from cache when feature flag enabled
- [ ] `survivor_status` correctly reflects strike limit status
- [ ] Feature flag implemented for gradual rollout
- [ ] Unit tests written with >80% coverage for new functions
- [ ] Integration tests cover full scoring → cache → scoreboard flow
- [ ] Performance benchmark met: scoreboard < 200ms
...
- **Impact**: Users see incorrect scores/strikes on scoreboard
...
**Risk 2: Performance Degradation During Update**
- **Impact**: Slow response times during 15-minute update window
- **Probability**: Low (updates are background process, documents are tiny)
- **Mitigation**:
...
- **Probability**: Low (proper indexes in place, small documents)
- **Mitigation**:
...
- **Mitigation**:
  - Display `lastUpdated` timestamp on scoreboard
...
- **Mitigation**:
  - Cache only updated via scoring pipeline or authenticated admin endpoints
...
- Decision to scope cache to scoreboard only (simplicity; results/profile remain on live queries)
...
2. **Cache Update Logic**
   ...
   - Write `buildPlayerCacheDocument()` — read stats from `league_memberships`, user info from `users`
   - Write `recalculatePlayerRanks()` with tie handling
   - Derive `survivor_status` from total strikes vs league strike limit
...
4. **Scoreboard API Route Update**
   - Update scoreboard API to read from cache (with feature flag)
   - Maintain fallback to live query when flag is off

5. **Backfill Script & Admin Tools**
...
**Add Monitoring For:**
- Cache update duration (alert if > 5 seconds)
...
- Scoreboard page load times (before/after cache implementation)
