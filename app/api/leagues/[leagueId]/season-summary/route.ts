...
import { getSeasonSummary } from '@/lib/db'
import { createApiResponse, handleApiError } from '@/lib/api-types'
import { verifyLeagueMembership } from '@/lib/auth-utils'

// GET /api/leagues/[leagueId]/season-summary - Get season summary with prizes and standings
export async function GET(
  ...
  try {
    ...
    const summaryData = await getSeasonSummary(leagueId)

    return Response.json(createApiResponse(true, summaryData))
  } catch (error) {
