...
router = APIRouter(prefix="/api/league-seasons/{league_season_id}", tags=["results"])


@router.get("/results")
async def league_results(league_season_id: str, request: Request) -> dict:
    """Port of app/api/leagues/[leagueId]/results/route.ts:6-32."""
    await require_league_membership(request, league_season_id)
    results_data = await get_league_results(league_season_id)
    return ok(results_data.model_dump())


@router.get("/scoreboard")
async def league_scoreboard(league_season_id: str, request: Request) -> dict:
    """Port of app/api/leagues/[leagueId]/scoreboard/route.ts:6-32."""
    await require_league_membership(request, league_season_id)
    scoreboard_data = await get_scoreboard_with_picks(league_season_id)
    return ok({
...
async def league_season_summary(league_season_id: str, request: Request) -> dict:
    """Port of app/api/leagues/[leagueId]/season-summary/route.ts:6-32."""
    await require_league_membership(request, league_season_id)
    summary_data = await get_season_summary(league_season_id)
    return ok(summary_data.model_dump())


@router.get("/players/{user_id}/profile")
async def player_profile(league_season_id: str, user_id: str, request: Request) -> dict:
    ...
    profile = await get_player_profile(league_season_id, user_id)
    if profile is None:
